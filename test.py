...
def filter_explosive_stocks(df: pd.DataFrame, range_day=10, stable_rate=4/3, min_volume=500000) -> pd.DataFrame:
    """ 找出符合條件的股票：
    ...
    def get_moving_averages(code: str):
        ...
        try:
            ...
            df = stock.history(period="25d")  # 取最近 25 天數據（確保有 25 天數據可計算）

            if df.empty or len(df) < 25:
                print(f"無法獲取 {ticker} 25天的數據")
                return None, None
    ...
    for i, row in df.iterrows():
        ...
        # 判斷前 1 天的 20MA 成交均量是否大於 N 股
        if prev_days.iloc[-1]["20VolumeMA"] > min_volume:
            ...
            if all(prev_days["Volume"] < prev_days[f"{range_day}VolumeMA"] * stable_rate):
                ...
                if latest["Volume"] > latest[f"{range_day}VolumeMA"] * (stable_rate*3/2):
